...
def best_fit_distribution(data, v, bins=200, ax=None):
    ...
    '''
    DISTRIBUTIONS = [st.genlogistic, st.beta, st.gamma, st.gumbel_r, st.gumbel_l,st.johnsonsb, st.johnsonsu, st.norm
    ]
    '''
    # Best holders
    ...
    for distribution in DISTRIBUTIONS:
        ...
        try:
            ...
            pass
    return (best_distribution.name, best_params, best_distribution, all_sse)
...
def loop():
   # vals = ["1","5", "10", "15", "20", "30"]
    ...
    #  dataYLim = ax.get_ylim()
        ...
        best_fit_name, best_fir_paramms, distribution, all_sse = best_fit_distribution(data, v, 200, None)
        best_dist = getattr(st, best_fit_name)
        '''
        f = open('./sse.csv', 'a')
        f.write(','.join(map(str, all_sse)))
        f.write(" ")
        f.close()
        '''
        # Update plots
